...
fn test_celsius_to_newton() {
    let temp = Temperature::new(WATER_FREEZING_C, Scale::Celsius);
    assert_approx_eq(temp.to_newton().value, 0.0);

    let temp = Temperature::new(WATER_BOILING_C, Scale::Celsius);
    assert_approx_eq(temp.to_newton().value, 33.0);
}

#[test]
fn test_newton_to_celsius() {
    let temp = Temperature::new(0.0, Scale::Newton);
    assert_approx_eq(temp.to_celsius().value, WATER_FREEZING_C);

    let temp = Temperature::new(33.0, Scale::Newton);
    assert_approx_eq(temp.to_celsius().value, WATER_BOILING_C);
}

#[test]
fn test_celsius_to_delisle() {
    let temp = Temperature::new(WATER_FREEZING_C, Scale::Celsius);
    assert_approx_eq(temp.to_delisle().value, 150.0);

    let temp = Temperature::new(WATER_BOILING_C, Scale::Celsius);
    assert_approx_eq(temp.to_delisle().value, 0.0);
}

#[test]
fn test_delisle_to_celsius() {
    let temp = Temperature::new(150.0, Scale::Delisle);
    assert_approx_eq(temp.to_celsius().value, WATER_FREEZING_C);

    let temp = Temperature::new(0.0, Scale::Delisle);
    assert_approx_eq(temp.to_celsius().value, WATER_BOILING_C);
}

#[test]
fn test_celsius_to_romer() {
    let temp = Temperature::new(WATER_FREEZING_C, Scale::Celsius);
    assert_approx_eq(temp.to_romer().value, 7.5);

    let temp = Temperature::new(WATER_BOILING_C, Scale::Celsius);
    assert_approx_eq(temp.to_romer().value, 60.0);
}

#[test]
fn test_romer_to_celsius() {
    let temp = Temperature::new(7.5, Scale::Romer);
    assert_approx_eq(temp.to_celsius().value, WATER_FREEZING_C);

    let temp = Temperature::new(60.0, Scale::Romer);
    assert_approx_eq(temp.to_celsius().value, WATER_BOILING_C);
}
...
fn test_scale_from_str() {
    ...
    assert_eq!("newton".parse::<Scale>().unwrap(), Scale::Newton);
    assert_eq!("n".parse::<Scale>().unwrap(), Scale::Newton);
    assert_eq!("delisle".parse::<Scale>().unwrap(), Scale::Delisle);
    assert_eq!("d".parse::<Scale>().unwrap(), Scale::Delisle);
    assert_eq!("romer".parse::<Scale>().unwrap(), Scale::Romer);
    assert_eq!("ro".parse::<Scale>().unwrap(), Scale::Romer);
